...
param (
    ...
    [string]$ArtifactDownloadUrl = 'https://github.com/mitsuhiko/insta/releases/download/1.48.0',
...
$app_version = '1.48.0'
if ($env:CARGO_INSTA_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:CARGO_INSTA_INSTALLER_GHE_BASE_URL
} elseif ($env:CARGO_INSTA_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:CARGO_INSTA_INSTALLER_GITHUB_BASE_URL
} else {
  $installer_base_url = "https://github.com"
}
if ($env:CARGO_INSTA_DOWNLOAD_URL) {
  $ArtifactDownloadUrl = $env:CARGO_INSTA_DOWNLOAD_URL
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrl = $env:INSTALLER_DOWNLOAD_URL
} else {
  $ArtifactDownloadUrl = "$installer_base_url/mitsuhiko/insta/releases/download/1.48.0"
}
$auth_token = $env:CARGO_INSTA_GITHUB_TOKEN
...
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\cargo-insta"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\cargo-insta"
}

if ($env:CARGO_INSTA_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set CARGO_INSTA_NO_MODIFY_PATH=1 in the environment"
}

if ($env:CARGO_INSTA_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:CARGO_INSTA_UNMANAGED_INSTALL
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:CARGO_INSTA_INSTALL_DIR)) {
    $force_install_dir = $env:CARGO_INSTA_INSTALL_DIR
