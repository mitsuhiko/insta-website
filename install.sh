...
if [ -n "${CARGO_INSTA_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$CARGO_INSTA_INSTALLER_GHE_BASE_URL"
else
    INSTALLER_BASE_URL="${CARGO_INSTA_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
if [ -n "${CARGO_INSTA_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$CARGO_INSTA_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$INSTALLER_DOWNLOAD_URL"
else
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/mitsuhiko/insta/releases/download/1.48.0"
fi
if [ -n "${CARGO_INSTA_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$CARGO_INSTA_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${CARGO_INSTA_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$CARGO_INSTA_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${CARGO_INSTA_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$CARGO_INSTA_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${CARGO_INSTA_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${CARGO_INSTA_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${CARGO_INSTA_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.30.2"},"source":{"app_name":"cargo-insta","name":"insta","owner":"mitsuhiko","release_type":"github"},"version":"1.48.0"}
EORECEIPT
...
RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/cargo-insta"

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
cargo-insta-installer.sh

The installer for cargo-insta 1.48.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/mitsuhiko/insta/releases/download/1.48.0
...
USAGE:
    cargo-insta-installer.sh [OPTIONS]
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set CARGO_INSTA_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "cargo-insta-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="26f081f5bf62d1dc03aae9c4137188d66a2debe6bcaf2884f2153fcb6eda27cb"
            _bins="cargo-insta"
            _bins_js_array='"cargo-insta"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "cargo-insta-x86_64-apple-darwin.tar.xz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="e2d6d6fa4bcb30f7d4834f6254a6da7f2982a9e9a3882695aca5e83d64231267"
            _bins="cargo-insta"
            _bins_js_array='"cargo-insta"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "cargo-insta-x86_64-pc-windows-msvc.zip")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".zip"
            _checksum_style="sha256"
            _checksum_value="7e23929588aaf5daff1e93bfc4addca7ccf55195444e84552b6588c9d864642e"
            _bins="cargo-insta.exe"
            _bins_js_array='"cargo-insta.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            ...
            _updater_bin=""
            ;;
        "cargo-insta-x86_64-unknown-linux-gnu.tar.xz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="1c05a480a5a7f755f0ea15b2d8e2f71ad51b9c3a270d38ac72005c57ac0a1487"
            _bins="cargo-insta"
            _bins_js_array='"cargo-insta"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "cargo-insta-x86_64-unknown-linux-musl.tar.xz")
            _arch="x86_64-unknown-linux-musl-static"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="703aa7d151ae59cac0bf8e7c2c27da4aca8070f3cb178f624ae10621ef77f5f5"
            _bins="cargo-insta"
            _bins_js_array='"cargo-insta"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="cargo-insta-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="cargo-insta-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="cargo-insta-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="cargo-insta-x86_64-unknown-linux-gnu.tar.xz"
            ...
            _archive="cargo-insta-x86_64-unknown-linux-musl.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${CARGO_INSTA_INSTALL_DIR:-}" ]; then
        _force_install_dir="$CARGO_INSTA_INSTALL_DIR"
